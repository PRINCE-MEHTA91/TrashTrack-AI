...
export default function ForgotPasswordPage() {
  ...
  const validate = (val: string) => {
    ...
    if (!re.test(val.trim())) return 'Enter a valid email address or phone number.'
    return ''
  }
  ...
  return (
    <AuthLayout
      ...
      topLinkTo="/login"
      heroHeadline={<>Reset your <span className="text-primary-400">TrashTrack AI</span> password</>}
    ...
    >
      {step === 'request' ? (
        <>
          ...
          <form id="forgot-password-form" onSubmit={handleSubmit} noValidate aria-label="Forgot password form" className="space-y-4">
            <FormField id="forgot-email" label="Email or Phone Number" error={touched ? error : undefined}>
              <TextInput
                id="forgot-email" type="text" name="email"
                ...
                hasError={touched && !!error} disabled={isLoading}
                leftIcon={<Mail className="w-4 h-4" />}
              />
            </FormField>

            <button id="forgot-submit" type="submit" disabled={isLoading} aria-busy={isLoading}
          ...
          <p className="mt-6 text-center text-sm text-gray-500">
            <Link to="/login" id="forgot-back-to-login"
        ...
        <div className="text-center py-4">
          ...
          <Link to="/login" id="forgot-success-login"
