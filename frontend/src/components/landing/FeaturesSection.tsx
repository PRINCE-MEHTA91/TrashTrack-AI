...
import {
  Brain, MapPin, Users, BarChart3, Bell, Camera, ShieldCheck, Zap,
...
export default function FeaturesSection() {
  ...
  /* Update arrow visibility and dot index on scroll */
    ...
    <section id="features" className="py-20 md:py-28 overflow-hidden">
      <div className="section-container">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-10">
          <div className="max-w-xl">
            ...
            <p className="text-gray-400 text-base">
              End-to-end — from citizen report to verified cleanup — powered by AI.
            </p>
          </div>
          ...
          <div className="hidden sm:flex items-center gap-2 shrink-0">
            <button
              id="features-scroll-left"
              onClick={() => scroll('left')}
              disabled={!canScrollLeft}
              aria-label="Scroll features left"
              className="w-10 h-10 rounded-full border border-surface-border flex items-center justify-center
                         text-gray-400 hover:text-white hover:border-primary-500 hover:bg-surface-muted
                         disabled:opacity-30 disabled:cursor-not-allowed transition-all duration-200"
            >
              <ChevronLeft className="w-5 h-5" />
            </button>
            <button
              id="features-scroll-right"
        ...
        <div className="flex items-center justify-between mt-5">
          {/* Dots */}
          <div className="flex items-center gap-1.5" role="tablist" aria-label="Feature card position">
            {FEATURES.map((f, i) => (
              <button
                key={f.title}
                role="tab"
