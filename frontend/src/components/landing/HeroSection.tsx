      ...
      <div className="section-container relative z-10 py-32 md:py-40">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-gray-400 text-lg sm:text-xl md:text-2xl leading-relaxed mb-10 max-w-2xl mx-auto animate-slide-up delay-200">
            TrashTrack AI connects citizens, field workers, and municipalities on one
            intelligent platform — from waste report to verified cleanup, every step tracked.
          </p>
          ...
          <div className="flex flex-wrap justify-center gap-3 animate-slide-up delay-400">
            {BADGES.map(({ icon: Icon, text }) => (
              <span key={text} className="flex items-center gap-2 text-sm text-gray-400 bg-surface-muted border border-surface-border rounded-full px-4 py-2">
          ...
          <div className="glass-card p-1 shadow-glow">
            <div className="bg-surface-card rounded-xl p-6 space-y-4">
              ...
              <div className="flex items-center justify-between">
                <span className="text-xs text-gray-500 font-medium">LIVE COMPLAINT FEED</span>
                <span className="flex items-center gap-1.5 text-xs text-primary-400">
                  <span className="w-1.5 h-1.5 rounded-full bg-primary-400 animate-pulse" />
                  3 active
                </span>
              </div>
              {/* Mock complaint cards */}
              {[
                { id: 'TT-2847', type: 'Plastic Waste', severity: 'HIGH',   status: 'IN PROGRESS', ward: 'Ward 12', time: '2 min ago',  color: 'text-amber-400',   dot: 'bg-amber-400'  },
                { id: 'TT-2846', type: 'Mixed Garbage', severity: 'MEDIUM', status: 'ASSIGNED',     ward: 'Ward 7',  time: '15 min ago', color: 'text-blue-400',    dot: 'bg-blue-400'   },
                { id: 'TT-2845', type: 'Organic Waste', severity: 'LOW',    status: 'VERIFIED',     ward: 'Ward 3',  time: '1 hr ago',   color: 'text-primary-400', dot: 'bg-primary-400'},
              ].map(c => (
                <div
                  key={c.id}
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-gray-500 text-xs">{c.id} · {c.ward}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 shrink-0">
                    <span className={`text-xs font-semibold ${c.color} hidden sm:block`}>{c.severity}</span>
                    <span className="badge bg-surface border border-surface-border text-gray-400 text-xs hidden md:flex">{c.status}</span>
