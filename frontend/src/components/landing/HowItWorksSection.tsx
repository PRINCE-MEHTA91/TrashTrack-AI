import { Camera, MapPin, Brain, UserCheck, CheckCircle2, Star } from 'lucide-react'
...
export default function HowItWorksSection() {
  return (
    <section id="how-it-works" className="py-24 md:py-32 relative">
      ...
      <div className="absolute inset-0 bg-radial-glow opacity-40 pointer-events-none" aria-hidden="true" />
      ...
      <div className="section-container relative z-10">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <p className="text-gray-400 text-lg">
            A transparent, auditable 6-step process that keeps every stakeholder informed
            at each stage of the waste lifecycle.
          </p>
        </div>
          ...
          {STEPS.map((step) => (
            ...
            >
              <div className="flex items-start gap-4 mb-4">
                <div className={`w-12 h-12 rounded-2xl ${step.bg} flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform`}>
        ...
        <div className="mt-16 card p-6 overflow-x-auto">
          ...
          <div className="flex items-center gap-2 min-w-max">
            {[
              ...
              { label: 'PENDING ASSIGNMENT', color: 'bg-amber-500/20 text-amber-400'  },
              { label: 'ASSIGNED',           color: 'bg-blue-500/20 text-blue-400'    },
              { label: 'IN PROGRESS',        color: 'bg-cyan-500/20 text-cyan-400'    },
              { label: 'CLEANUP SUBMITTED',  color: 'bg-indigo-500/20 text-indigo-400'},
              { label: 'PENDING VERIFY',     color: 'bg-orange-500/20 text-orange-400'},
              { label: 'VERIFIED',           color: 'bg-primary-500/20 text-primary-400' },
              { label: 'CLOSED',             color: 'bg-primary-700/30 text-primary-300' },
            ].map((s, i, arr) => (
              <div key={s.label} className="flex items-center gap-2">
                <span className={`badge ${s.color} whitespace-nowrap text-xs`}>{s.label}</span>
                {i < arr.length - 1 && (
