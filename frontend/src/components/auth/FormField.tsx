...
/* ──────────────────────────────────────────────────────────
   FormField – label + input wrapper + error message
────────────────────────────────────────────────────────── */
...
export function FormField({ id, label, error, children }: FormFieldProps) {
  return (
    <div className="space-y-1">
      <label htmlFor={id} className="block text-xs font-semibold text-gray-300 uppercase tracking-wide">
        {label}
      </label>
      {children}
      {error && (
...
/* ──────────────────────────────────────────────────────────
   TextInput – styled input with optional left icon
────────────────────────────────────────────────────────── */
  ...
  ({ id, hasError, leftIcon, rightSlot, className = '', ...rest }, ref) => {
    return (
      <div className="relative flex items-center">
        {leftIcon && (
          <span className="absolute left-3.5 text-gray-500 pointer-events-none" aria-hidden="true">
            {leftIcon}
          </span>
        )}
        <input
          ...
          className={`
            ...
            ${hasError
              ...
              : 'border-white/15 focus:border-primary-500 focus:ring-1 focus:ring-primary-500/30 hover:border-white/25'
            }
            ${className}
          `}
          {...rest}
...
export function OrDivider() {
  return (
    <div className="relative flex items-center gap-3 my-4">
      ...
      <span className="text-gray-500 text-xs font-medium shrink-0">or continue with</span>
      <span className="flex-1 h-px bg-surface-border" />
    </div>
  )
}
